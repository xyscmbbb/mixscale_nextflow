...
# No row.names = 1 here. A pair CSV whose first column IS the cell column (as
# JL114_guide_csv.csv is: cell,pair_key,target_gene) would have that column
# consumed as row names, and cell_col would then be reported as missing. read.csv
# still takes a leading unnamed index column as row names on its own, so the
# CSVs that do carry one keep working.
pair_df <- read.csv(opt$pair_csv, check.names = FALSE)
...
if (enc %in% c("csr_matrix", "csc_matrix")) {
  ...
  # py_to_r() on a Python int goes through a 32-bit conversion, so a file with
  # more than 2^31-1 nonzeros comes back WRAPPED and negative (2,846,132,814 was
  # read as -1,448,834,482). That silently passed the cap check below and then
  # died in integer(n) with "invalid 'length' argument". Round-trip through a
  # string so the value survives, whatever its size.
  nnz    <- as.numeric(py_to_r(bi$str(Xg[["indices"]]$shape[[0]])))
  ...
  if (enc == "csr_matrix" && subset_rows) {
    ...
    ip_ds  <- Xg[["indptr"]]
    # Cast in numpy, not in R: indptr's last entry is the file's total nnz, which
    # is past int32 here, and reticulate would wrap or NA it on the way over.
    p_all  <- as.numeric(py_to_r(np$asarray(ip_ds)$astype("float64")))
